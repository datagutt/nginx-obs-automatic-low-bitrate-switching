...
class Chat {
  ...
  onMessage(message) {
    if (message !== null) {
      ...
      switch (parsed.command) {
        ...
        case "HOSTTARGET":
          if (_config.default.twitchChat.enableAutoStopStreamOnHostOrRaid && !parsed.message.startsWith("-") && this.obs.bitrate != null) {
  ...
  rec(bool) {
    ...
    if (!bool) {
      this.say("[REC] ".concat(this.obs.obsRecording ? this.locale.rec.started : this.locale.rec.stopped));
      return;
    }
  ...
  startStopRec(bool) {
    ...
    return _asyncToGenerator(function* () {
      if (bool) {
        try {
          if (!_this3.obs.obsRecording) {
            _this3.obs.toggleRecording();

            _this3.say("[REC] ".concat(_this3.locale.rec.started));

            log.success("Started recording");
          } else {
            (0, _stringTemplate.default)("[REC] ".concat(_this3.locale.rec.error), {
        ...
        try {
          if (_this3.obs.obsRecording) {
            ...
            _this3.say("[REC] ".concat(_this3.locale.rec.stopped));
  ...
  switch(sceneName) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var res = (0, _fastFuzzy.search)(sceneName, _this4.obs.scenes, {
        keySelector: obj => obj.name
      });
      var scene = res.length > 0 ? res[0].name : sceneName;

      try {
        yield _this4.obs.switchScene(scene);
  ...
  refresh() {
    ...
    return _asyncToGenerator(function* () {
      if (!_this5.isRefreshing) {
        try {
          var lastScene = _this5.obs.currentScene;
          if (lastScene == null) return _this5.say(_this5.locale.refresh.error);
          yield _this5.obs.switchScene(_config.default.obs.refreshScene);

          _this5.say(_this5.locale.refresh.success);

          _this5.isRefreshing = true;
          setTimeout(() => {
            _this5.obs.switchScene(lastScene);
